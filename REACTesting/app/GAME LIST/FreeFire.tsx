...
const FreeFire = () => {
  const route = useRoute<RouteProp<RootStackParamList, 'FreeFire'>>();
  ...
  const checkFreeFireId = async () => {
    ...
    const options = {
      method: 'GET',
      url: `https://id-game-checker.p.rapidapi.com/ff-global/${userId}`,
      headers: {
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.title}>Free Fire ID Checker</Text>
      <TextInput
        style={styles.input}
      ...
      />
      <Button title="Check ID" onPress={checkFreeFireId} />
      {result && (
        <View style={styles.resultContainer}>
          {result.error ? (
            ...
            <>
              <Text style={styles.resultText}>UID: {result.userId}</Text>
